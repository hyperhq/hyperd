...
func NewSession(client *http.Client, authConfig *cliconfig.AuthConfig, endpoint *Endpoint) (r *Session, err error) {
	...
	if endpoint.VersionString(1) != IndexServerAddress() && endpoint.URL.Scheme == "https" {
		...
		if info.Standalone && authConfig != nil {
			glog.V(1).Infof("Endpoint %s is eligible for private registry. Enabling decorator.", endpoint.String())
...
func (r *Session) GetRemoteHistory(imgID, registry string) ([]string, error) {
	...
	glog.V(1).Infof("Ancestry: %v", history)
	return history, nil
}
...
func (r *Session) GetRemoteImageLayer(imgID, registry string, imgSize int64) (io.ReadCloser, error) {
	...
	for i := 1; i <= retries; i++ {
		...
		glog.V(1).Infof("Error contacting registry %s: %v", registry, err)
		if res != nil {
			if res.Body != nil {
				res.Body.Close()
			}
...
func (r *Session) GetRemoteTags(registries []string, repository string) (map[string]string, error) {
	...
	for _, host := range registries {
		...
		glog.V(1).Infof("Got status code %d from %s", res.StatusCode, endpoint)
		defer res.Body.Close()
...
func (r *Session) PushImageJSONIndex(remote string, imgList []*ImgData, validate bool, regs []string) (*RepositoryData, error) {
	...
	glog.V(1).Infof("[registry] PUT %s", u)
	glog.V(1).Infof("Image list pushed to index:\n%s", imgListJSON)
	...
	for {
		...
		glog.V(1).Infof("Redirected to %s", u)
	}
	defer res.Body.Close()
	...
	if !validate {
		...
		glog.V(1).Infof("Auth token: %v", tokens)
...
func (r *Session) SearchRepositories(term string) (*SearchResults, error) {
	glog.V(1).Infof("Index server: %s", r.indexEndpoint)
