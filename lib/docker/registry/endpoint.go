...
func (e *Endpoint) pingV1() (RegistryInfo, error) {
	glog.V(1).Infof("attempting v1 ping for registry endpoint %s", e)
	...
	if err := json.Unmarshal(jsonString, &info); err != nil {
		glog.V(1).Infof("Error unmarshalling the _ping RegistryInfo: %s", err)
		// don't stop here. Just assume sane defaults
	}
	if hdr := resp.Header.Get("X-Docker-Registry-Version"); hdr != "" {
		glog.V(1).Infof("Registry version header: '%s'", hdr)
		info.Version = hdr
	}
	glog.V(1).Infof("RegistryInfo.Version: %q", info.Version)

	standalone := resp.Header.Get("X-Docker-Registry-Standalone")
	glog.V(1).Infof("Registry standalone header: '%s'", standalone)
	...
	glog.V(1).Infof("RegistryInfo.Standalone: %t", info.Standalone)
	return info, nil
}

func (e *Endpoint) pingV2() (RegistryInfo, error) {
	glog.V(1).Infof("attempting v2 ping for registry endpoint %s", e)
