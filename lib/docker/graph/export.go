...
func (s *TagStore) ImageExport(imageExportConfig *ImageExportConfig) error {
	...
	addKey := func(name string, tag string, id string) {
		glog.V(1).Infof("add key [%s:%s]", name, tag)
	...
	for _, name := range imageExportConfig.Names {
		...
		glog.V(1).Infof("Serializing %s", name)
		...
		glog.V(1).Infof("End Serializing %s", name)
	}
	...
	if len(rootRepoMap) > 0 {
		...
		glog.V(1).Infof("There were no repositories to write")
	}
	...
	glog.V(1).Infof("End export image")
	return nil
}
...
func (s *TagStore) exportImage(name, tempdir string) error {
	for n := name; n != ""; {
		...
		if written != len(imageInspectRaw) {
			glog.Warningf("%d byes should have been written instead %d have been written", written, len(imageInspectRaw))
		}
