...
func createTarFile(path, extractDir string, hdr *tar.Header, reader io.Reader, Lchown bool) error {
	...
	case tar.TypeXGlobalHeader:
		glog.V(1).Infof("PAX Global Extended Headers found and ignored")
		return nil
	...
	go func() {
		...
		for _, include := range options.IncludeFiles {
			filepath.Walk(filepath.Join(srcPath, include), func(filePath string, f os.FileInfo, err error) error {
				if err != nil {
					glog.V(1).Infof("Tar: Can't stat file %s to tar: %s", srcPath, err)
					return nil
				}
				...
				if include != relFilePath {
					...
					if err != nil {
						glog.V(1).Infof("Error matching %s", relFilePath, err)
						return err
					}
				}
				...
				if err := ta.addTarFile(filePath, relFilePath); err != nil {
					glog.V(1).Infof("Can't add file %s to tar: %s", filePath, err)
				}
				return nil
			})
		}
		...
		if err := ta.TarWriter.Close(); err != nil {
			glog.V(1).Infof("Can't close tar writer: %s", err)
		}
		if err := compressWriter.Close(); err != nil {
			glog.V(1).Infof("Can't close compress writer: %s", err)
		}
		if err := pipeWriter.Close(); err != nil {
			glog.V(1).Infof("Can't close pipe writer: %s", err)
		}
	}()
...
func (archiver *Archiver) TarUntar(src, dst string) error {
	glog.V(1).Infof("TarUntar(%s %s)", src, dst)
...
func (archiver *Archiver) CopyWithTar(src, dst string) error {
	...
	glog.V(1).Infof("Creating dest directory: %s", dst)
	if err := system.MkdirAll(dst, 0755); err != nil && !os.IsExist(err) {
		return err
	}
	glog.V(1).Infof("Calling TarUntar(%s, %s)", src, dst)
...
func (archiver *Archiver) CopyFileWithTar(src, dst string) (err error) {
	glog.V(1).Infof("CopyFileWithTar(%s, %s)", src, dst)
	srcSt, err := os.Stat(src)
	if err != nil {
		return err
	}
