...
func (devices *DeviceSet) ensureImage(name string, size int64) (string, error) {
	...
	if _, err := os.Stat(filename); err != nil {
		...
		glog.V(1).Infof("Creating loopback file %s for device-manage use", filename)
...
func (devices *DeviceSet) deviceFileWalkFunction(path string, finfo os.FileInfo) error {
	...
	if strings.HasSuffix(finfo.Name(), ".migrated") {
		glog.V(1).Infof("Skipping file %s", path)
		return nil
	}
	...
	glog.V(1).Infof("Loading data for file %s", path)
	...
	if dinfo.DeviceId > MaxDeviceId {
		glog.Errorf("Ignoring Invalid DeviceId=%d", dinfo.DeviceId)
		return nil
	}
	...
	glog.V(1).Infof("Added deviceId=%d to DeviceIdMap", dinfo.DeviceId)
	return nil
}

func (devices *DeviceSet) constructDeviceIdMap() error {
	glog.V(1).Infof("[deviceset] constructDeviceIdMap()")
	defer glog.V(1).Infof("[deviceset] constructDeviceIdMap() END")

	var scan = func(path string, info os.FileInfo, err error) error {
		if err != nil {
			glog.V(1).Infof("Can't walk the file %s", path)
			return nil
		}
...
func (devices *DeviceSet) unregisterDevice(id int, hash string) error {
	glog.V(1).Infof("unregisterDevice(%v, %v)", id, hash)
	info := &DevInfo{
		Hash:     hash,
		DeviceId: id,
	}
	...
	if err := devices.removeMetadata(info); err != nil {
		glog.V(1).Infof("Error removing metadata: %s", err)
		return err
	}
...
func (devices *DeviceSet) registerDevice(id int, hash string, size uint64, transactionId uint64) (*DevInfo, error) {
	glog.V(1).Infof("registerDevice(%v, %v)", id, hash)
	info := &DevInfo{
		Hash:          hash,
		DeviceId:      id,
		Size:          size,
...
func (devices *DeviceSet) createRegisterDevice(hash string) (*DevInfo, error) {
	...
	if err := devices.openTransaction(hash, deviceId); err != nil {
		glog.V(1).Infof("Error opening transaction hash = %s deviceId = %d", hash, deviceId)
	...
	for {
		if err := devicemapper.CreateDevice(devices.getPoolDevName(), deviceId); err != nil {
			if devicemapper.DeviceIdExists(err) {
				...
				glog.Errorf("Device Id %d exists in pool but it is supposed to be unused", deviceId)
				deviceId, err = devices.getNextFreeDeviceId()
				if err != nil {
					return nil, err
				}
			...
			glog.V(1).Infof("Error creating device: %s", err)
			devices.markDeviceIdFree(deviceId)
			return nil, err
		}
		break
	...
	glog.V(1).Infof("Registering device (id %v) with FS size %v", deviceId, devices.baseFsSize)
...
func (devices *DeviceSet) createRegisterSnapDevice(hash string, baseInfo *DevInfo) error {
	...
	for {
		if err := devicemapper.CreateSnapDevice(devices.getPoolDevName(), deviceId, baseInfo.Name(), baseInfo.DeviceId); err != nil {
			...
			glog.V(1).Infof("Error creating snap device: %s", err)
			devices.markDeviceIdFree(deviceId)
			return err
		}
		break
	...
	if _, err := devices.registerDevice(deviceId, hash, baseInfo.Size, devices.OpenTransactionId); err != nil {
		devicemapper.DeleteDevice(devices.getPoolDevName(), deviceId)
		devices.markDeviceIdFree(deviceId)
		glog.V(1).Infof("Error registering device: %s", err)
		return err
	}
...
func (devices *DeviceSet) rollbackTransaction() error {
	glog.V(1).Infof("Rolling back open transaction: TransactionId=%d hash=%s device_id=%d", devices.OpenTransactionId, devices.DeviceIdHash, devices.DeviceId)
...
func (devices *DeviceSet) processPendingTransaction() error {
	...
	if devices.OpenTransactionId < devices.TransactionId {
		glog.Errorf("Open Transaction id %d is less than pool transaction id %d", devices.OpenTransactionId, devices.TransactionId)
		return nil
	}
...
func (devices *DeviceSet) closeTransaction() error {
	if err := devices.updatePoolTransactionId(); err != nil {
		glog.V(1).Infof("Failed to close Transaction")
		return err
	}
	return nil
}
...
func determineDriverCapabilities(version string) error {
	...
	glog.V(1).Infof("devicemapper: driver version is %s", version)
...
func (devices *DeviceSet) removeDevice(devname string) error {
	...
	glog.V(1).Infof("[devmapper] removeDevice START(%s)", devname)
	defer glog.V(1).Infof("[devmapper] removeDevice END(%s)", devname)
...
func (devices *DeviceSet) cancelDeferredRemoval(info *DevInfo) error {
	...
	glog.V(1).Infof("[devmapper] cancelDeferredRemoval START(%s)", info.Name())
	defer glog.V(1).Infof("[devmapper] cancelDeferredRemoval END(%s)", info.Name)
...
func (devices *DeviceSet) Shutdown() error {
	glog.V(1).Infof("[deviceset %s] Shutdown()", devices.devicePrefix)
	glog.V(1).Infof("[devmapper] Shutting down DeviceSet: %s", devices.root)
	defer glog.V(1).Infof("[deviceset %s] Shutdown() END", devices.devicePrefix)
	...
	for _, info := range devs {
		...
		if info.mountCount > 0 {
			...
			if err := devices.deactivateDevice(info); err != nil {
				glog.V(1).Infof("Shutdown deactivate %s , error: %s", info.Hash, err)
			}
			devices.Unlock()
		}
		info.lock.Unlock()
	...
	if info != nil {
		...
		devices.Lock()
		if err := devices.deactivateDevice(info); err != nil {
			glog.V(1).Infof("Shutdown deactivate base , error: %s", err)
		}
		devices.Unlock()
		info.lock.Unlock()
	}
	...
	if devices.thinPoolDevice == "" {
		if err := devices.deactivatePool(); err != nil {
			glog.V(1).Infof("Shutdown deactivate pool , error: %s", err)
		}
	}
...
func (devices *DeviceSet) UnmountDevice(hash string) error {
	glog.V(1).Infof("[devmapper] UnmountDevice(hash=%s)", hash)
	defer glog.V(1).Infof("[devmapper] UnmountDevice(hash=%s) END", hash)
	...
	glog.V(1).Infof("[devmapper] Unmount(%s)", info.mountPath)
	if err := syscall.Unmount(info.mountPath, syscall.MNT_DETACH); err != nil {
		return err
	}
	glog.V(1).Infof("[devmapper] Unmount done")
