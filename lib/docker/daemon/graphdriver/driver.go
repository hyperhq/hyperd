...
func New(root string, options []string) (driver Driver, err error) {
	for _, name := range []string{os.Getenv("HYPER_STORAGE_DRIVER"), DefaultDriver} {
		if name != "" {
			glog.V(1).Infof("[graphdriver] trying provided driver %q", name) // so the logs show specified driver
		...
		for _, prior := range priorDrivers {
			...
			if prior == name {
				...
				if err != nil {
					...
					glog.Errorf("[graphdriver] prior storage driver %q failed: %s", name, err)
					return nil, err
				}
				if err := checkPriorDriver(name, root); err != nil {
					return nil, err
				}
				glog.Infof("[graphdriver] using prior storage driver %q", name)
				return driver, nil
			}
		}
	}
