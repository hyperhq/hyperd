...
func (a *Driver) Remove(id string) error {
	...
	if a.active[id] != 0 {
		glog.Errorf("Removing active id %s", id)
	}
...
func useDirperm() bool {
	enableDirpermLock.Do(func() {
		...
		if err != nil {
			glog.Errorf("error checking dirperm1: %v", err)
			return
		}
		...
		if err := Unmount(union); err != nil {
			glog.Errorf("error checking dirperm1: failed to unmount %v", err)
		}
	})
