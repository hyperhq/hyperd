...
func (t *TrustStore) reload() error {
	...
	if len(statements) == 0 {
		if t.autofetch {
			glog.V(1).Infof("No grants, fetching")
	...
	glog.V(1).Infof("Reloaded graph with %d grants expiring at %s", len(grants), expiration)
...
func (t *TrustStore) fetch() {
	...
	for bg, ep := range t.baseEndpoints {
		...
		if err != nil {
			glog.Infof("Trust graph fetch failed: %s", err)
			continue
		}
		b, err := statement.Bytes()
		if err != nil {
			glog.Infof("Bad trust graph statement: %s", err)
			continue
		}
		// TODO check if value differs
		if err := ioutil.WriteFile(path.Join(t.path, bg+".json"), b, 0600); err != nil {
			glog.Infof("Error writing trust graph statement: %s", err)
		}
		fetchCount++
	}
	glog.V(1).Infof("Fetched %d base graphs at %s", fetchCount, time.Now())

	if fetchCount > 0 {
		go func() {
			if err := t.reload(); err != nil {
				glog.Infof("Reload of trust graph failed: %s", err)
			}
		}()
