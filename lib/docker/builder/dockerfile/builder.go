...
type Builder struct {
	*Config
	...
	Name        string
	Hyperdaemon *hyperdaemon.Daemon
...
// TODO: remove?
func BuildFromConfig(d *daemon.Daemon, config *runconfig.Config, changes []string) (*runconfig.Config, error) {
	...
	b, err := NewBuilder(d, nil, nil, nil, nil)
	if err != nil {
		return nil, err
	}
	b.runConfig = config
