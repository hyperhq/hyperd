...
func (cli *HyperClient) HyperCmdReplace(args ...string) error {
    ...
    if oldPodId == "" {
        return fmt.Errorf("Please provide the old pod which you want to replase!")
    }
    ...
    if newPodId != "" {
       ...
       fmt.Printf("Successful to replace the old pod(%s) with new pod(%s)\n", oldPodId, newPodId)
       return nil
    }
    if newPodFile != "" {
        ...
        fmt.Printf("Successful to replace the old pod(%s) with new pod file(%s)\n", oldPodId, newPodFile)
        return nil
    }
    return nil
}
